...
char __ptsname_buffer[64];
char *ptsname (int fd)
{ 
    gramado_system_call ( 808, 
       (unsigned long) fd,                     // fd do master.
       (unsigned long) &__ptsname_buffer[0],   // buffer em ring3 para o nome.
       (unsigned long) 64 );

    return __ptsname_buffer;
}
...
{ 
    ...
    len_ret = (int) gramado_system_call ( 809, 
                        (unsigned long) fd,       // fd for master.
                        (unsigned long) buf,      // buffer for the name.
                        (unsigned long) buflen ); // size of the buffer. 
